...
| M4 FSDP2 | `COMPLETE` | M4.1 正确性/Rank 失败、M4.2 原子 DCP/失败矩阵、M4.3 Qwen3-8B 四卡 Probe、50 Step、Step 25→50 Resume、显存与导出证据 | 无；8 卡、ZeRO-3、Changed World Size 与吞吐为非阻塞增强项 |
| M5 双模式正式后训练 | `IN_PROGRESS` | 已开始 M5.0 GQA + Thinking/Non-thinking ADR、配置与 Template 契约 | PR 合并后进入 Reasoning Pilot/Dev 数据；不把契约或 M4 Smoke 当作质量结果 |
| M6–M8 | `NOT_STARTED` | M2 冻结评测可供后续使用 | M5 真实候选和对应里程碑门禁 |
...
严格依赖：M1–M3。执行批次：FSDP2 `fully_shard`/Activation Checkpointing → DCP 分片保存 →
...
- M5.0：冻结 GQA + Thinking/Non-thinking 双模式 ADR、Schema 和 Template；M2 历史身份不变。
- M5.1：构建可验证 Reasoning Pilot、200 条独立 Dev 和双模式注册数据。
...
完成门禁：双模式数据/训练/评测/血缘/Checkpoint/Adapter/Model Card 真实且可复现；未通过
M6 Promotion Gate 时模型保持 `Development`。详细契约见 `docs/m5_sft_contract.md`。

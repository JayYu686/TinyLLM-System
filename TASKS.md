...
| M2 数据与评测前置 | `IN_PROGRESS` | 固定来源/许可导入、规范化/Dedup/Split、Qwen3 Mask、Token 平衡/Packing/Manifest，以及不可变 Registry 合成 Smoke | #40 仍需在干净提交上执行完整固定源构建；随后进入 #11 污染检查与冻结 Baseline |

...
只有完成上述 Smoke 的候选版本才可写成“已支持”。M4 环境不得静默改变 M2 数据构建环境的
Tokenizer 版本。
...
正确性；随后隔离 `.venv-m4` 通过 FSDP2/DCP/Qwen API、Tiny Qwen 前后向和 CPU/Gloo
复查，并在干净提交 `6c241a0` 上通过单卡 BF16 CUDA/NCCL FSDP2 Smoke。2026-07-17
又在干净提交 `7ce1b96` 上使用物理 GPU 6、7 通过双卡 BF16 CUDA/NCCL、两个
`TransformerBlock` 的 Activation Checkpointing、完整 Shard 覆盖和 Rank 1 中途退出诊断。
M4.1 最小正确性证据当时已经齐备；该阶段结束时 M4 保持 `IN_PROGRESS`，下一门禁为 M4.2 DCP。
详见 [M4.1 CPU/Gloo 报告](../reports/m4/fsdp2_cpu_correctness.md)、
[M4 CUDA 就绪性报告](../reports/m4/fsdp2_cuda_readiness.md)与
[M4.1 双卡报告](../reports/m4/fsdp2_multigpu_activation_failure.md)。
...
当前证据：2026-07-18 已在干净提交 `cf0f96c` 上完成两进程 CPU/Gloo DCP 正式对照。
无中断与 Step 2 中断后新进程恢复的 Run 均到达 Step 6，最终完整参数 Hash 与每步指标逐条
一致；坏分片、缺 Marker、缺 Rank、错误 World Size、数据版本和配置漂移均被拒绝。
M4.2 已完成，详见 [M4.2 DCP 报告](../reports/m4/fsdp2_dcp_recovery.md)。该证据不替代
M4.3 Qwen3-8B BF16 四卡恢复门禁。
...
M4 的 Exact Resume 成功条件是：恢复进程通过完整性和兼容性检查，加载模型、Optimizer、
Scheduler、RNG 与 Sampler Cursor，从 Step 25 的下一训练位置继续，并以连续的 Step 1–50
指标到达 Step 50。M4.2 的 CPU/Gloo 无中断对照承担逐位一致性证明；M4.3 没有设置四卡
未中断数值对照门禁，因此不得声称 BF16 结果与未中断 Run 逐位或容差等价。如需该数值结论，
必须另行冻结重复基线和事前容差，不能在看到结果后调整。

## 8. 观测与报告

机器可读 Artifact 至少记录：

- 每 Step Loss、LR、Gradient Norm、Tokens 和非有限值状态；Phase Wall Clock 必须单独记录，
  未采集逐 Step Duration 时不得发布训练吞吐；
...
当前证据：2026-07-20 已在干净提交 `c8f2b00` 上使用物理 GPU 5–8 完成固定 Revision
Qwen3-8B 四卡 Probe、50 Step、Step 25→50 新进程 Exact Resume、两个完整 DCP Checkpoint
和独立 Safetensors 加载。M4 状态为 `COMPLETE`，详见
[M4.3 四卡正式报告](../reports/m4/fsdp2_qwen3_8b_formal.md)。

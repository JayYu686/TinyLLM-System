...
复查，并在干净提交 `6c241a0` 上通过单卡 BF16 CUDA/NCCL FSDP2 Smoke。由于单卡 NCCL
不验证跨卡通信，且双卡请求被忙卡 Preflight 正确拒绝，Activation Checkpointing、Rank
中途退出和真实多卡 CUDA/NCCL 仍未完成，M4.1 保持 `IN_PROGRESS`。详见
[M4.1 CPU/Gloo 报告](../reports/m4/fsdp2_cpu_correctness.md)与
[M4 CUDA 就绪性报告](../reports/m4/fsdp2_cuda_readiness.md)。

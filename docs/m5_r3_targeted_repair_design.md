...
真实正式来源门禁通过后，精确 Tokenization 证明 160 条选择一轮只有 4,933 个监督 Token，
因此 150K Targeted Token 与四次总使用上限在数学上不可同时满足。该矛盾在任何 R3 训练
或 Dev 评测前发现并修订为 `m5-r3-mixture-v2`：

- 从 218 条合格来源中按任务族、语言和诊断标签重新选择 160 条；
- 不改变 700K / 150K / 150K Token 预算；
- 真实重平衡来源每轮提供 5,037 个监督 Token；
- 单来源总使用上限冻结为 30，真实构建范围为 29–30；
- 不读取 M6 冻结结果，也不重新生成 Teacher 来源。

原四次上限作为失败的 v1 设计证据保留，不静默覆盖。详见
[Mixture v2 构建报告](../reports/m5/m5_r3_mixture.md)。

...
R3 Candidate 的 v3 结果触发了自动诊断。该结论由自动评分即可确定，无需额外补写
...
可见文本重建后的真实 GPU 全量复验达到：Non-thinking 自动正确 `147/260`、JSON Valid
`79/80`、格式有效 `300/300`、泄漏 `0/300`；Thinking 自动正确 `135/260`、JSON Valid
`78/80`、格式有效 `297/300`、自然闭合 `296/300`、强制闭合 `4/300`、泄漏 `0/300`。
Thinking 相比隐藏 Pad 路径的 `59/260` 已恢复，剩余机械缺口仅为 1 条 JSON。

两条 JSON 失败中，一条重复输出 `</think>`，属于格式失败并继续拒绝；另一条同时缺失一个
右花括号并把两个公开必需顶层键误缩进 `data`。v2 已分别实现补右花括号和必需键提升，但不会

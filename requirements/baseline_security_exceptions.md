...
| `CVE-2026-9856` | Transformers 4.57.6 | The affected `save_pretrained()` path consumes attacker-controlled chat-template names. The Baseline is inference-only, never calls tokenizer or processor `save_pretrained()`, and accepts only the pinned, hash-verified local Qwen3 text tokenizer. | Remove after Transformers 5.10+ passes the frozen Baseline compatibility gate, or immediately if tokenizer export or arbitrary Hub repositories enter this profile. |

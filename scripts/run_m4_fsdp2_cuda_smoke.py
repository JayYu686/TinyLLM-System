#!/usr/bin/env python3
"""Run one fail-closed M4 Tiny Model CUDA/NCCL Smoke on explicit idle GPUs."""
...
from tinyllm.training import FSDP2TrainingResult, load_fsdp2_config
...
def run_smoke(
    *,
    ...
    """Preflight selected GPUs, retain private logs, and validate Rank-zero evidence."""
    ...
    if config.distributed.backend != "nccl" or config.distributed.device_type != "cuda":
        raise RuntimeError("M4 GPU Smoke requires backend=nccl and device_type=cuda")
    if config.precision.dtype != "bf16":
        raise RuntimeError("M4 RTX 3090 GPU Smoke requires BF16")
    ...
    if git_dirty:
        raise RuntimeError("formal M4 GPU Smoke requires a clean Git worktree")
    ...
    try:
        ...
        raise RuntimeError("M4 FSDP2 GPU Smoke timed out") from exc
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(f"M4 FSDP2 GPU Smoke failed with exit code {completed.returncode}")
    try:
        ...
        raise RuntimeError("FSDP2 worker emitted an invalid Rank-zero result") from exc
...
def build_parser() -> argparse.ArgumentParser:
    """Build the bounded M4 CUDA/NCCL Smoke interface."""
...
def main() -> int:
    ...
    try:
        ...
        print(f"M4 FSDP2 GPU Smoke failed: {exc}", file=sys.stderr)
        return 1

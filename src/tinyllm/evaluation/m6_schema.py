...
class M6OutputControlConfig(StrictSchema):
    ...
    nonthinking_stop_policy: Literal["truncate-before-first-thinking-tag-v1"] | None = Field(
        default=None,
        exclude_if=lambda value: value is None,
    )
    nonthinking_stop_strings: tuple[Literal["<think>"], Literal["</think>"]] | None = Field(
        default=None,
        exclude_if=lambda value: value is None,
    )
    ...
    def validate_json_decoder(self) -> M6OutputControlConfig:
        ...
        stop_control = (self.nonthinking_stop_policy, self.nonthinking_stop_strings)
        if any(value is not None for value in stop_control) and any(
...
class M6ReleaseConfig(StrictSchema):
    ...
    def validate_tasks(self) -> M6ReleaseConfig:
        ...
        if self.protocol_version == "m6-release-v6" and (
            self.domain_execution.output_control is None
            or self.domain_execution.output_control.json_repair_policy != "json-syntax-only-v3"
            or self.domain_execution.output_control.json_decoder_id != "xgrammar-json-shape-v1"
            or self.domain_execution.output_control.json_decoder_version != "0.2.4"
...
class M6DomainTranscript(StrictSchema):
    ...
    nonthinking_tag_truncated: bool = Field(
        default=False,
    ...
    def validate_transcript(self) -> M6DomainTranscript:
        ...
        if self.nonthinking_tag_truncated and (
            self.mode != "nonthinking"
            or not any(tag in self.response for tag in ("<think>", "</think>"))
...
class M6DomainPassSummary(StrictSchema):
    ...
    nonthinking_truncated_items: int = Field(
        default=0,
        ge=0,
        le=300,
        exclude_if=lambda value: value == 0,

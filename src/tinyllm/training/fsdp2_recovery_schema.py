"""Strict Rank-zero result for one M4.2 DCP recovery phase."""
...
class FSDP2RecoveryResult(StrictSchema):
    """One successful or intentionally interrupted FSDP2 invocation."""
    ...
    def validate_identity_and_progress(self) -> FSDP2RecoveryResult:
        """Bind Run, config, checkpoint, metrics, and recovery mode."""
        ...
        if self.durable_metric_records != self.global_step:
            raise ValueError("metrics must contain one durable row per optimizer step")
